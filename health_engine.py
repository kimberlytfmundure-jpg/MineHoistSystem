...
def priority_level(health_score: float, fault_probability: float) -> str:
    """
    Priority mirrors the same health-score bands as decision_status, so the
    two labels shown on a card are always consistent (e.g. 'Schedule
    maintenance' will always pair with 'High', never 'Critical').

    fault_probability is intentionally NOT used to override this banding:
    the fault classifier tends to report very high confidence (often
    95-100%) even for components that are only mildly degraded, since the
    synthetic training data is highly separable. Letting that raw
    confidence override the health-based banding would make almost every
    degrading component show as 'Critical' regardless of how much life it
    actually has left -- which is misleading. fault_probability is still
    shown on the card itself for context.
    """
    if health_score > HEALTH_THRESHOLDS["normal"]:
        return "Low"
    elif health_score > HEALTH_THRESHOLDS["monitor"]:
