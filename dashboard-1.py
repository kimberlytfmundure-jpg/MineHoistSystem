...
st.caption(
    ...
    "currently running on synthetic sensor data until live sensor feeds "
    "are connected."
)
...
with tab_perf:
    ...
    st.info(
        "Model performance is based on synthetic training data and will be "
        "recalibrated once live sensor data becomes available."
    )
...
with tab_impact:
    ...
    st.markdown("---")
    st.subheader("Downtime Reduction Estimate")
    st.warning(
        "Figures below are based on industry-typical downtime durations and "
        "will be calibrated with site-specific maintenance data."
    )
