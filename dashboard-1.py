...
def train(df):
    ...
    # Proposal Stage 4: compare Decision Tree / Random Forest / Gradient
    # Boosting to justify which algorithm the production classifier uses.
...
with tab_live:
    ...
    # First pass: compute every assessment before rendering, so we can show
    # an alerts banner (workflow step 12: "send alerts when required") above
    ...
    if critical_components:
        ...
        if st.session_state.get("sent_alerts"):
            with st.expander(f"Sent alert log ({len(st.session_state.sent_alerts)})"):
                st.dataframe(pd.DataFrame(st.session_state.sent_alerts), use_container_width=True)
        st.caption(
            "This simulates the spec's 'send alerts when required' workflow step. "
            "The button above stands in for a real notification -- to wire up an "
            "actual email or SMS alert, add an API call here (e.g. via smtplib "
            "for email or a provider like Twilio for SMS) using credentials you "
            "supply; no code elsewhere needs to change."
        )
    else:
...
with tab_perf:
    ...
    st.caption(
        ...
        "Gradient Boosting to determine which gives the highest prediction "
        "accuracy for fault classification. Results below, trained and "
        "tested on the same data split as the rest of this dashboard."
    )
    ...
    st.dataframe(algo_df.style.format({"Accuracy (%)": "{:.2f}"}), use_container_width=True)
    st.success(
